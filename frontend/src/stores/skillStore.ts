...
import {
  defaultSkillTree,
  normalizeConnections as baseNormalizeConnections,
  normalizeNodes as baseNormalizeNodes,
  normalizeSkillTree as baseNormalizeSkillTree,
  normalizeStatus,
...
export const normalizeConnections = baseNormalizeConnections
export const normalizeNodes = baseNormalizeNodes
export const normalizeSkillTree = baseNormalizeSkillTree
...
const toSerializableSkillTree = (tree: SkillTree): SkillTree => {
  // structuredClone は SW 経由で Proxy を含むケースで失敗することがあるため、JSON 経由でシリアライズする
  ...
  actions: {
    ...
    async applyImportedSkillTree(tree: SkillTree) {
      ...
      try {
        ...
        console.warn('スキルツリーの取得に失敗しました。新規として登録します', error)
      }
      ...
      if (existing) {
        ...
        if (incomingVersion < existingVersion) {
          const confirmed = window.confirm('同じIDのスキルツリーが既に存在します。上書きしますか？')
          if (!confirmed) {
     ...
     * 処理名: スキルツリー読込
     *
     * 処理概要: Service Worker経由のローカルストレージからスキルツリーを取得し、正規化したうえで状態に反映する。
     *
     * 実装理由: 初期表示やリロード時にブラウザ内の最新データを取得し、オフラインでも動作させるため。
     */
     ...
     * 処理名: スキルツリー保存
     *
     * 処理概要: 接続を正規化したうえでService Worker経由でローカル保存する。
     *
     * 実装理由: 編集内容をブラウザ内に永続化し、ネットワーク不要で再利用できるようにするため。
     */
    async saveSkillTree() {
      try {
        await ensureServiceWorker()
     ...
     * 処理名: スキルツリーエクスポート
     *
     * 処理概要: 接続を正規化してローカル保存中のデータを取得し、Blob経由でクライアントにダウンロードさせる。
     *
     ...
     * 処理名: スキルツリーインポート
     *
     * 処理概要: ファイルからJSONを読み込み正規化し、ローカルストレージへ保存して状態を置き換える。
     *
    ...
    async activateSkillTree(treeId: string) {
      ...
      this.clearSelection()
      // UIの再描画後に最新状態を取得して同期する
      void this.refreshFromSourceUrl()
    },
    /**
     * 処理名: ステータス読込
     *
     * 処理概要: Service Worker経由で残ポイントとアンロックスキルIDを取得し、正規化して状態に反映する。
     *
     * 実装理由: ブラウザ内のステータスを最新化し、オフラインでも整合性を保つため。
     */
     ...
     * 処理名: 進行状況保存
     *
     * 処理概要: 現在の残ポイントとアンロックスキルIDをService Worker経由でローカル保存する。
     *
     * 実装理由: ユーザー進行をブラウザ内に保持し、ネットワーク不要で復元できるようにするため。
     */
    async saveProgress() {
      try {
        await ensureServiceWorker()
    ...
    async deleteSkillTree(treeId: string) {
      ...
      if (!targetId) {
        return { ok: false, message: 'スキルツリーIDを入力してください' }
      }
    ...
    async refreshFromSourceUrl() {
      ...
      try {
        ...
        console.error('スキルツリーURLからの最新取得に失敗しました', error)
      }
    },
  },
})
